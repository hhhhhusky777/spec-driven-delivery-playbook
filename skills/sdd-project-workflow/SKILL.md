...
- Before final pull-request review, make the candidate converge every tracked
  ...
  merge.
- Begin every delivery in an isolated worktree and feature branch created from
  the accepted target. Check for a newer playbook revision and synchronize it
  in that worktree before whiteboard or implementation work. A maintenance-only
...
- Treat the branch point as the implementation baseline. Do not routinely
  ...
  candidate with its target before final review. If the baseline cannot support
  safe progress, follow the canonical error-handling authority.
