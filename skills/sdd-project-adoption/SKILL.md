---
name: sdd-project-adoption
description: Adopt or resume the Spec-Driven Delivery Playbook in a project whose adoption manifest is absent or incomplete, using the installer-generated agent guide and preserving independent review gates.
---

# SDD Project Adoption

## Governing goals

After runtime verification, read and apply
`docs/documentation-quality-policy.md#five-goals-and-agent-judgment` from the
verified playbook checkout. Keep operational choices within project authority.

## Exception routing

Read and apply `docs/batch-review-and-recovery.md#recovery-without-restarting-everything`
and `docs/batch-review-and-recovery.md#exception-triage-and-upstream-reporting`
from the verified checkout. Use `templates/reviews/exception-triage.md` in the
existing recovery record. Runtime-verification failure still forbids reading
an unverified checkout or making prohibited project writes.

## Optional batched route

Before using an explicitly approved batch route, read
`docs/batch-review-and-recovery.md` from the verified read-only playbook checkout.
Within that scope, assemble dependent drafts and the neutral whiteboard for one
installation review instead of the separate per-artifact stops in steps 7–16.
Record each mapped control and preserve final human acceptance, verified
INSTALLED transition and runtime handoff. Outside valid authority, use the
ordinary sequence below.

Resolve source paths through that verified checkout, not the installed skill
directory. Availability is not project adoption or additional authority.

Use this skill only through the machine-local `.sdd-runtime/agent-guide.md`
generated by `install-sdd.sh`. The guide supplies the verified project root,
playbook checkout, immutable revision, manifest path, and cleanup record.

## Execution

1. Read the complete generated guide and verify its project root, checkout Git
   origin, and resolved revision. Treat the checkout as read-only. If a binding
   is missing or mismatched, make no project edit and report `BLOCKED`.
2. Read the pinned adoption runbook, manifest template, agent-adoption trigger,
   project instructions, and existing project authorities required for the
   current adoption state.
3. If the manifest is absent, instantiate only the manifest at the guide's
   recorded path. Populate its durable repository, immutable revision, and
   materialization mode from the guide. Never copy a machine-local path into a
   project contract.
4. Perform the dependency-ready adoption action permitted by the manifest and
   its exact allowed write scope. Apply only the pre-approved
   `EXPLICIT_REVIEW`, `AUTO_CONTINUE`, or `REVIEW_ON_EXCEPTION` mode; an absent
   or invalid mode fails closed to `EXPLICIT_REVIEW`. Preserve unrelated and
   user-owned work.
   Before entering manifest state `BLOCKED`, record the current non-blocked
   state in `State before block`; reset it to `None` only after returning to a
   reviewed safe state. Never guess this value.
5. Before proposing `REUSE` for an existing policy, prove decision-level
   conformance against every applicable obligation in the pinned template and
   record the project evidence in the manifest. File existence, similar
   headings, and observed Git history are insufficient. Route a missing or
   ambiguous decision to `UPDATE_EXISTING`. Require a reviewed exception with
   rationale, owner, risk, and equivalent control for intentional differences.
   Apply this to every applicable policy family. Update the canonical existing
   authority; never create a parallel or replacement policy merely because it
   is incomplete. If competing copies lack declared precedence, record
   `BLOCKED`. Do not silently copy a playbook default or choose for the project
   owner.
6. For an automatic mode, verify approved/current inputs, no new semantic
   decision, every declared gate passing on the output revision, no unresolved exception
   or drift, and an auditable result. Continue only through the approved
   automation boundary and stop at the next mandatory semantic checkpoint.
   `AUTO_CONTINUED` is not approval. Initial discovery, authority mapping,
   policy changes, authorized policy exceptions, activation, and adoption approval remain
   `EXPLICIT_REVIEW`.
7. Compare each action's changed facts, links, commands, and availability
   claims with previously approved artifacts. Record affected artifacts as
   `STALE` in the manifest freshness register and schedule the earliest
   dependency-ready correction. In `EXPLICIT_REVIEW`, the immediate next action
   remains review of the current change and no second artifact is edited. In an
   automatic mode, any newly stale or unknown impact stops the automation
   segment. Keep volatile adoption progress in the manifest instead of copying
   it into stable entry points.
8. Run the project checks applicable to the changed artifact and report exact
   evidence. Do not substitute playbook-repository checks for project evidence.
9. Before every review gate, apply `templates/reviews/agent-self-review.md`
   from the verified checkout to the exact candidate. Record
   `SELF_REVIEW_PASSED` or `SELF_REVIEW_FAILED`, not approval.
10. Apply `templates/reviews/fresh-context-agent-review.md` from the verified
   checkout. It owns reviewer isolation, retained seats, exact receipts,
   findings and correction rounds; use the existing adoption review record.
11. After fresh-context `APPROVED`, stop for mandatory human adoption review.
   Fresh review prepares that decision and cannot replace human authority.
12. Reach the action's actual review or automation boundary, not a stop after
   each internal edit. Use the canonical error-handling framework for failures;
   do not equate a safely correctable agent mistake with a policy exception.
   Record completed automatic actions separately; required gates remain.
13. Continue on a later invocation from the human-reviewed manifest state. Never
   approve your own artifact or infer reviewer authorization.
14. Before proposing `MAPPED -> INSTALLED`, require every applicable freshness
   entry to be `CURRENT` and complete final cross-document verification.
15. After recorded authority moves the manifest to `INSTALLED`, instantiate an
   empty solution whiteboard at the guide's recorded adoption root. Populate
   neutral control values such as `EMPTY`, `Not recorded`, or `None`; do not
   infer a need, requirement, solution, handoff, plan, or delivery result.
16. Stop when the empty whiteboard and installed manifest have passed their
   applicable documentation checks.
17. After fresh-context and human approval of that boundary, do not admit a need through a
   guide that selects this adoption skill. If its cleanup is `PENDING`, report
   the required runtime handoff: from the project root, run
   `./install-sdd.sh --cleanup`, then `./install-sdd.sh`, and verify that the new
   guide selects `sdd-project-workflow` at the manifest's pinned revision. Do
   not clean up while adoption work or empty-whiteboard review remains open.

## Completion report

Report the current manifest state, actions completed and their review modes,
files changed, automatic-gate/audit evidence, stale artifacts or `None`,
blockers, stop reason, required reviewer, and whether the empty whiteboard
exists. When the adoption boundary is complete, also report whether cleanup and
workflow-runtime installation are required before the first need.
