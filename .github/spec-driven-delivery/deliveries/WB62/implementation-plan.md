...
| Key information; C04–C05 | T01: Git-first retrievable review evidence with concise findings and justified unique evidence | Git review history, unavailable bytes, unique logs and checker snapshot cases | No historical deletion or arbitrary volume threshold |
...
| Self-review candidate revision | WB62-P01-R01; exact identity in review record |
...
controls join the same PR. Exclude runtime #33/#34/#36, schema/validator behavior
changes, review quorum/approval invalidation changes, new dependencies, blanket
auto-merge, retained-history pruning and activation of other projects.
...
| Q02 | Change schemas or mandatory approvals? | No; report design amendment if required | RESOLVED | Owner |
...
| C05 | sdd-lifecycle.mjs reads optional Reviewed snapshot and masks enumerated controls | Keep required local comparison input; explain necessity | Existing immutable-snapshot tests; no code/schema change |
...
| Depends on | None |
| Data phase | NONE |
| Source boundary | Whiteboard source inventory: docs, Contributing, README, CHANGELOG, three source skills, review/affected phase templates, generated guidance in install-sdd.sh, one maintained example and corresponding tests; U84 project controls |
| Compatibility before/after | Machine schemas and installer runtime logic unchanged; installed project upgrades explicit |
