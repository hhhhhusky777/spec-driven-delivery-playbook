...
| `DR03` | The feature should extend the existing human brief without adding a document, gate, or mandatory helper program. | Accepted |
...
| `DP02` | Counts come from the exact candidate against its actual PR target; each changed file belongs to one primary category. | Binary or otherwise non-line-countable files remain explicit instead of fabricated. | Tests assert the reusable contract and semantic review checks examples. |
