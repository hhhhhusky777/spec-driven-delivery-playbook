...
| `DR04` | Completed tasks need bounded evidence before review without deferring required test implementation. | Define focused tests as only the tests covering changed files and lines, then retain exact-head two-agent review. |
| `DR05` | Repeating full validation on task candidates wastes time. | Reserve full validation for the reviewed final candidate that will merge back to the protected integration branch. |
| `DR06` | Terms such as heavy or long-running leave the boundary ambiguous. | Use only focused tests and full validation as the two named gates. |
...
| `DR08` | Existing PR automation still ran the full source suite before review. | Split PR focused validation from manually dispatched, scheduled, and target full validation. |
...
| `D03` | Every task implements its required tests, then runs only focused tests covering changed files and lines before unchanged two-agent review. | The definition is the boundary; do not add testing categories between focused tests and full validation. | Review guidance and regression agree. |
| `D04` | Full validation runs only after review of the final candidate that will merge back to the protected integration branch. | A single-task PR to `main` is already final. | Policy, workflow, contributing guide, and README diagram agree. |
| `D05` | Candidate corrections repeat focused tests and the same reviewers; final-candidate corrections also invalidate full validation. | Stricter project policy and human merge authority remain intact. | Tests lock both correction paths. |
...
| `D07` | Source automation enforces focused-tests-then-review-then-full-validation ordering. | PR events run focused validation; non-PR final/target runs use the full suite. | Workflow and regression agree. |
